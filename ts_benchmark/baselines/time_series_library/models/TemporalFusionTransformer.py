...
datatype_dict = {
    ...
    '桔子洲2': TypePos(static=[],observed=[x for x in range(9)]),
    '三角洲2': TypePos(static=[],observed=[x for x in range(9)]),
    '捞刀河2': TypePos(static=[], observed=[x for x in range(9)]),
    '桔子洲': TypePos(static=[],observed=[x for x in range(9)]),
    '三角洲': TypePos(static=[],observed=[x for x in range(9)]),
    '捞刀河': TypePos(static=[], observed=[x for x in range(9)]),
}
...
class TemporalFusionTransformer(nn.Module):
    def __init__(self, configs):
        ...
        if configs.data in datatype_dict:
            ...
            self.static_len = 0
            self.observed_len = getattr(configs, "enc_in", 7)
